...
- [x] **T1.2** Remote Terraform state backend landed (design v2.8, 2026-05-21). State now lives in Hetzner Object Storage via the S3-compatible API; `prevent_destroy` on `lifecycle` plus server-side encryption (SSE-S3) cover the in-place threat. New files: `terraform/backend.tf` (empty-block partial config with the static skip_*/use_path_style/encrypt flags), generated `terraform/backend.tfbackend` (operator-specific bucket/endpoint/region/credentials; gitignored), `infrastructure.terraform_state` section in `seed.template.yaml` with `TerraformStateConfig` in `seed_schema.py`, `tf-backend` target in `seed_bootstrap.py`. Makefile gained `tf-init` (now passes `-backend-config=backend.tfbackend`), `tf-init-reconfigure` (for S3-key rotations), and `tf-snapshot` (state pull → dated file for the offline kit). DEPLOY.md §0 documents the Console-only bucket-creation and S3-credential minting paths (Hetzner Cloud API does not expose either); §2 lists the new seed fields; §2a secrets-location table gains an entry for the state-backend access/secret keys; §3 updates the init command. Phase 2 of T1.2 (offline-kit snapshot) is operational via `make tf-snapshot` — operator runs it after every successful apply, copies to FDE USB, and shreds the local copy per the printed instructions. Files: `terraform/backend.tf`, `seed.template.yaml`, `scripts/seed_schema.py`, `scripts/seed_bootstrap.py`, `Makefile`, `docs/DEPLOY.md`, `.gitignore`.
